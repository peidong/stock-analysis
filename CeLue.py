...
def 策略1(df, start_date='', end_date='', mode=None):
    ...
    if mode == 'fast':
        ...
        code_first_two = df['code'].iat[0][:2]  # 或 df['code'].iloc[0][:2]
        if code_first_two == "68" or code_first_two == "30":
            TJ04_1 = 1.2
        else:
            TJ04_1 = 1.1
...
def 策略2(df, HS300_信号, start_date='', end_date=''):
    ...
    for k, v in TJ06_MA60_DAY.items():
        prev_index = int(i - v)
        # print(f"prev_index={prev_index},i={i}, v={v}, k={k}")
        # 检查索引是否合法
        if 0 <= prev_index < len(MA60):
            TJ06_MA60.iat[i] = MA60.iat[prev_index]
        else:
            TJ06_MA60.iat[i] = np.nan  # 处理越界或无效索引
        i += 1
